...
def build_workbook(path: str, facility: pd.DataFrame, hist: pd.DataFrame, latest: pd.DataFrame,
    ...
    lines = [
        ...
        ["04_공사비DB_공종별", "대표 공고 기준 공종별 금액 구성(API+문서), 총공사비 수식"],
    ...
    tcols = ["프로젝트ID", "시설ID", "시설명", "사업유형", "공종", "공고번호", "공고차수", "공고명", "공고일시", "수요기관",
             "추정가격_API", "기초금액_API", "부가세(수식)", "도급자관급액_문서", "관급자관급액_문서", "총공사비(수식)",
    ...
    for c in tcols:
        if c not in t.columns:
            t[c] = None
    t = t[tcols]
    _write_df(ws, t, money_cols=["추정가격_API", "기초금액_API", "부가세(수식)", "도급자관급액_문서", "관급자관급액_문서",
                                 "총공사비(수식)", "추정가격_문서", "기초금액_문서"],
    ...
    for r in range(2, ws.max_row + 1):
        ...
        # 기초금액(API) → 추정가격(API)×1.1 → 기초금액(문서) → 추정가격(문서)×1.1 → 0, + 관급자재
        ws[f"{col['총공사비(수식)']}{r}"] = (f'=IF(ISNUMBER({B}),{B},IF(ISNUMBER({P}),{P}*1.1,IF(ISNUMBER({Bd}),{Bd},IF(ISNUMBER({Pd}),{Pd}*1.1,0))))'
                                        f'+N({G1})+N({G2})')
    ...
    n = ws.max_row + 2
    ws[f"A{n}"] = ("주: 건설공사비지수 보정계수는 사용자 입력(기준연도 지수/공고연도 지수). 총공사비 = 기초금액(API, 없으면 추정가격×1.1, 그것도 없으면 문서 추출값) + 도급자관급액 + 관급자관급액. "
