# HANDOFF.md — g2b_costdb 진행 상황

> 매 세션 시작 시 이 파일을 먼저 읽는다. 갱신일: 2026-09-10

## 0. 2026-09-08 진행 (사용자 PC, C:\DB_WORK\g2b_costdb)
- 전용 저장소 github.com/seio0701/g2b_costdb 생성·클론, Python 3.14, 패키지 설치, `doctor`·두 테스트 통과, 인증키 설정 완료.
- **`probe --ym 2026-08` 성공**: 공고목록(totalCount 8,830)·기초금액(6,217) 모두 `[매핑 OK]`. 실제 응답에서 추가로 확인된 필드를
  config/코드에 반영: 관급자재(`govsplyAmt`, 도급자/관급자 설치 관급액), 부가세 `VAT`, 예산 `bdgtAmt`, 이전공고번호 `befBidBbancNo`,
  변경사유, 수요기관코드, 부공종명 1~9, 현장설명서 URL 1~5, 표준공고문 URL. 공고차수는 `000` 3자리.
- 면허제한 조회(`inqryDiv=2`)는 표본 공고에서 항목 없음 → 기본 비활성(`api.use_license_limit: false`), 공종은 주공종명·부공종명으로 분류.
- 호출량 추정 갱신: 월 9+7 페이지 → 93개월 약 1,500회(하루 950회 예산으로 2일).
- 다음: 세션 2(3개월 시험 수집). `git pull` 후 `config.yaml period` 를 2026-06~2026-08 로 바꾸고 `collect` → `discover`.

## 0-1. 2026-09-09 추가
- 3개월 후보 199건 검토 결과를 검색어·분류 규칙에 반영(오탐 36건 제외, 사업유형 29건 재분류, 시설명 12건 수정).
- S6 추출에 **배치 모드(`extract --batch`, 50% 할인)** 와 **파일 인수인계 모드(`extract --export/--import`, Cowork·사람이 채움)** 추가. 뒤 단계 동일.
- 다음: 사용자 PC 에서 `git pull` → `discover` 재실행 → (검수) → `research` → `dedup` → `attach` 까지 3개월치로 먼저 돌려 첨부 다운로드·HWP 해석을 확인,
  그 뒤 `extract --export` 로 30~50건을 Cowork 로 시범 추출해 품질 확인 → 본 처리는 `extract --batch --yes`.

## 0-2. 2026-09-09 낙찰정보(낙찰금액·낙찰률) 연동 — 코드 완료, 실제 API 미확인
- 사용자 질문 "최종 낙찰률 정보도 있나?" → 입찰공고정보서비스에는 낙찰하한율(`sucsfbidLwltRate`)만 있고 낙찰 결과는 별도 서비스인
  「조달청_나라장터 낙찰정보서비스」(공사 낙찰 목록 `getScsbidListSttusCnstwk`)에 있음. 사용자가 활용신청 후 알려주기로 함.
- 구현(기본 꺼짐 `api.use_awards: false`): `probe` 가 낙찰 표본·`[award_*]` 매핑 상태를 출력, `collect` 가 개찰일 기준 월 단위로
  `data/raw/award_YYYYMM.jsonl` → `data/award_all.parquet`, `discover` 가 공고번호+차수(→공고번호)로 붙여 `낙찰금액_API/낙찰률_API/낙찰자/참가업체수/낙찰개찰일시`
  컬럼 생성, Excel `02/03/04` 에 표시하고 `04` 에 `낙찰률(수식)`(낙찰금액/기초금액). 총공사비에는 사용하지 않음(참고 컬럼).
- 미확인 사항(실제 응답으로 확정 필요): End Point `api.award_base_url`(현재 `…/1230000/as/ScsbidInfoService` 추정), 기간 조회 파라미터
  `award_query.inqryDiv`(probe 가 1·2·3 을 시험), 필드명 `fields.award_*`(`sucsfbidAmt` 등 추정, 후보 자동 탐색).
- 다음(사용자가 활용신청 완료를 알리면): `git pull` → `config.yaml api.use_awards: true` → `probe --ym 2026-08` 출력의 낙찰 덩어리 확인 →
  필요 시 `award_base_url`/`award_query`/`fields.award_*` 수정 → `collect`(3개월이면 월 1~2페이지 추가) → `discover` 이후 동일.

## 0-3. 2026-09-10 본 수집 완료(세션 3) + discover 대용량 최적화
- 사용자 PC 에서 `collect` 완료: 2019-01~2026-09(92개월 + 진행 중인 9월) **공사 공고 1,294,182건 → `data/notices_all.parquet`**,
  **기초금액 1,025,708건 → `data/bsis_all.parquet`**. 3개월 시험 수집 때의 추정(월 8,800건)보다 월평균 약 14,000건으로 많음.
- 129만 건 규모를 합성 데이터로 재현해 `discover` 단계를 측정·최적화(샌드박스 4코어 기준, 사용자 PC 는 비슷하거나 조금 느릴 수 있음):
  전량 parquet 는 standardize 가 쓰는 컬럼만 읽음(145개 → 약 50개, 메모리 수 GB 절약), 후보 탐색은 공고명만 순회(iterrows 제거, 약 3배),
  재검색의 수요기관 판정은 고유값에만 적용(약 3배). 129만 행 실측(샌드박스): standardize 136초, 후보 탐색 48초, 재검색(시설 400개) 63초, 최대 메모리 3.9 GB.
- 다음(사용자 PC): `git pull` → `python -m g2b_costdb.pipeline discover`(수 분, `output/facility_candidates.xlsx` 생성. 기존 3개월 검수 내용은
  (시설키, 수요기관)이 같은 행에 이어짐) → Excel 검수 → `research` → `dedup` → `attach`(첨부 다운로드·HWP 해석률 확인) → `extract --export` 시범 → `excel`.
- 낙찰정보서비스(0-2절)는 사용자의 활용신청 결과 대기 중(`use_awards: false` 유지).

## 0-4. 2026-09-10 전량 후보(facility_candidates.xlsx, 4,148개 시설) 검토 → 규칙 보강
- 전량 `discover` 결과: 시설 4,148개(포함 1,578 / 제외 1,913 / 검토필요 657). 연도별 최초공고 2019년 850 … 2026년 248.
- 발견한 문제와 수정:
  | 문제 | 수정 |
  |---|---|
  | 시설 사업유형을 **최빈값**으로 정해 유지보수 공고가 많은 시설(예: '목포 종합경기장 건립공사' 1,802억)이 '제외' 기본값 — 177개 시설(합계 약 1.8조원) | 시설에 신축·증축·리모델링 공고가 **하나라도** 있으면 그 유형(우선순위 신축 > 증축·리모델링 > 증축 > 리모델링), 없을 때만 미분류/유지보수 최빈값 (`discover._facility_work_type`) |
  | 중첩 괄호·상태어만 남는 공고명('수의견적 제출 안내 공고[… (리모델링)공사 (전기공사)]')에서 시설명이 '제출 ]', '시설', '공고' 로 추출 | 괄호를 안쪽부터 반복 제거 + 최상위 괄호 문구 추출(`strip_brackets`, `bracket_contents`), 홀로 선 상태어(입찰·공고·제출·안내·견적…) 제거, 괄호 밖이 일반어(학교·시설·공사…)뿐이면 괄호 안 문구 사용, 연도 접두어('23년~24년', '25~26년', '26,27년') 제거, 짝 없는 괄호 제거 |
  | 두 어절 검색어('스마트팜 온실')의 시설명이 '스마트팜' 으로 잘림 | 검색어 나머지 어절이 이어지면 포함(공종·사업유형 어휘 제외) |
  | 지하철역·도로 공사('9호선 … 종합운동장역 레일교환', '관광호텔 앞 가로등', '종합운동장교차로')가 후보로 | `keywords.yaml exclude_common`(운동장역·레일교환·가로등·교통신호기·횡단보도·도시계획도로·군도·농로·교차로·급수공사·하수관…) + 운동시설·공연장 분류에 '호선' 제외('호선'은 환승주차장 이름에 필요해 공통 제외어에서 뺌) |
  | '설치공사' 미분류 657건 중 설비·부속물(차양막·그늘막·막구조·휀스·CCTV·스코어보드·전광판·흡음·비가림·보일러·난간·도배·장판·수목전지·보식…)이 검토필요로 남음 | 유지보수 어휘 추가, 부속어에 개설·잔디·관람석, 증축에 '확충' |
- 효과(업로드된 파일의 공고명 예시·사업유형 분포로 근사): 제외→포함 177, 검토필요→포함 85, 검토필요→제외 75, 제외어로 탈락 122(포함이던 11건 확인: 실제 시설 손실 없음, 환승주차장 1건은 '호선' 조정으로 유지).
  새 기본값 예상: 포함 약 1,800 / 제외 약 1,740 / 검토필요 약 470. 시설명이 달라지는 시설 약 110개(상태어·괄호 잔재 제거).
- 다음(사용자 PC): `git pull` → `discover` 재실행(기존 검수 값은 (시설키, 수요기관)이 같은 행에 유지되나, 시설명이 바뀐 약 110개는 새 행으로 나타남) → 검수.
  검수 요령: `추정가격_합계` 내림차순으로 '포함'과 '검토필요'를 먼저 보고, '제외'는 `사업유형_분포`에 신축·증축·리모델링이 없는 것이 확인되므로 건너뛰어도 됨.
  공동주택('영구임대'·'행복주택'처럼 단지명 없는 시설키)은 `검수_시설명`에 단지·지구명을 넣어야 재검색이 정확해짐.

## 0-5. 2026-09-10 재생성 파일(4,037개 시설) 확인 → 검수 기본값 이어받기 수정
- 재생성 결과 시설명·제외어는 반영됐으나 **검수_포함여부가 한 건도 바뀌지 않음**: `discover` 가 이전 파일의 검수 칸을 사용자 편집 여부와 상관없이
  그대로 이어받아, 옛 규칙의 기본값('제외')이 새 기본값('포함')을 덮음(평택아트센터 건립 4,310억·목포 종합경기장 건립 등 185개 시설).
- 수정: 이전 값이 **그 행의 기본값과 같으면(손대지 않은 칸)** 이어받지 않고 새 규칙 기본값으로 다시 계산. 사용자가 고친 값(기본값과 다른 포함여부·시설명·수요기관,
  별칭·메모)만 유지(`discover._user_edited`). 추가로 `discover --fresh`(시설ID만 유지, 검수 칸 전부 초기화) 옵션.
- 현재 사용자 파일에는 옛 기본값이 '편집된 값'처럼 남아 있는 행이 392개 있으므로, **검수를 시작하기 전인 지금 `discover --fresh` 로 한 번 초기화**해야 함.
  이후에는 일반 `discover` 로 재실행해도 고친 값만 이어진다.
- 분류 보강: '기계설비공사·건축설비공사'는 공종명이므로 부속어(설비) 판정에서 제외(온실건축 및 기계설비공사 → 유지보수 오분류 방지),
  검색어가 이름 앞부분인 시설('호텔인터시티')은 뒤 어절 보존, 숙박시설 제외어(호텔외식·호텔스쿨·계열·강의실).
- 새 기본값 예상(재생성 파일의 사업유형 기준): 포함 약 1,825 / 제외 약 1,767 / 검토필요 약 445.

## 0-6. 2026-09-10 `discover --fresh` 결과(4,132개 시설) 확인 — 검수 시작 가능
- 검수_포함여부가 전부 새 규칙 기본값과 일치(포함 1,840 / 제외 1,827 / 검토필요 465), 제외 시설 중 신축·증축·리모델링 공고가 있는 것 0개,
  시설ID 는 이전 파일과 겹치는 3,867개 행 모두 유지. 포함 상위는 송파 창의 공공주택(9,144억)·고덕강일(6,870억)·평택아트센터(4,310억)·목포종합경기장(1,802억).
- 검토필요 상위는 '충남미술관 및 공영주차장 건립'(2,724억)·'성남중원 행복주택 및 공영주차장 건설'처럼 복합 발주(주차장이 부속어로 걸림)와
  '건축공사'만 있는 공고(울산혁신지구 공공실버주택 238억) — 사람이 판단.
- 소폭 보강: 리모델링에 '현대화', 증축에 '증설', 유지보수에 방음·식재·임시주차장·임시포장·덧씌우기, 공통 제외어에 농어촌도로·도로건설·터널,
  시설명 앞의 '및/외/내/등' 제거('본교 및 서면 유리온실' → '서면 유리온실', 23건).
- 다음(사용자 PC): `git pull` → `discover`(일반 실행. 손대지 않은 칸만 새 기본값으로) → **Excel 검수 시작** → `research` → `dedup` → `attach`.

## 0-7. 2026-09-10 단지명 없는 시설명에 지자체명 접두(사용자 요청)
- 시설명이 검색어+일반어뿐이면(`classify.is_generic_facility_name`: '영구임대아파트', '공공주택지구', '종합운동장', '제2봉안당') 수요기관의
  가장 구체적인 시·군·구(`short_institution`: '전남광주통합특별시 구례군' → '구례군', 시·군·구가 없으면 첫 어절 '서울주택도시개발공사')를 앞에 붙인다.
  공동주택뿐 아니라 모든 분류에 적용(울산시설공단 종합운동장, 하남시 제2봉안당).
- `research` 는 접두어를 뗀 패턴('국민임대아파트')으로도 검색하고 `검수_수요기관` 필터로 범위를 좁히므로 재검색 결과는 이전과 같다(`strip_institution_prefix`).
- 시설키가 바뀌므로 해당 시설(약 수백 개)은 다음 `discover` 에서 새 시설ID를 받는다(아직 검수 전이라 영향 없음).

## 0-8. 2026-09-10 사용자 PC 에서 research → dedup → attach 실행(전량, 검수 후)
- `research`: 시설 1,842개 → 공고 6,584건(시설 1,784개). "재검색 결과 없음" 약 60개 시설 — 원인: 시설명은 괄호·상태어를 지운 이름이라
  원 공고명의 부분 문자열이 아닌 경우('송파 창의 공공주택' vs '송파 창의혁신 공공주택 건설사업', '′23∼′ 영덕군 …', '영덕농업테마파크 건축').
  → 수정: 정확한 이름으로 없으면 어절 사이 간격(15자)을 허용한 예비 검색(`discover._gap_pattern`), 연도 접두어 '′23∼′24년' 제거,
  기호뿐인 어절·끝의 접두어 조각('월배공원 재') 제거. 지자체 접두어를 뗀 2자 패턴('온실')은 수요기관 필터가 있으면 경고하지 않음.
- 같은 시설이 수요기관만 달라 두 시설로 잡힘(F0037 화성도시공사 / F0038 경기도 화성시 '화성 함백산추모공원') → 검수 파일에서 **시설ID 를 같게 적으면**
  `research` 가 한 시설로 병합(시설명·별칭·수요기관 합집합). 05 시트 이중 집계를 막으려면 검수에서 이런 쌍을 찾아 합쳐야 함.
- `dedup`: 6,584건 → 대표 4,117건(프로젝트 1,926개, 경고 1,041건). 경고 대부분은 예초·수목전정·디딤돌 같은 관리성 공고가 '기타' 공종으로 들어와
  소액 분리된 것 → 유지보수 어휘(예초·전정·제초·벌초·디딤돌·전원공급) 추가로 다음 research 에서 빠짐.
- `attach`(진행 중): zip 안의 .xls(구형 Excel) 미지원, 스캔 PDF 는 pytesseract 없어 OCR 생략, 손상 PDF 1건. 25건당 30~90초 → 4,117건에 2~3시간.
- HWP/HWPX 추출 실패는 파일이 `data/files/<공고번호>/` 에 남으므로 나중에 일괄 재시도 가능: 텍스트가 전혀 없는 공고는 `attach` 재실행 때 자동 재시도,
  다른 파일로 텍스트를 얻은 공고의 실패 HWP 까지 다시 하려면 `attach --retry-failed`(재처리한 공고의 옛 추출노트는 새 기록으로 대체).
  백엔드 순서: olefile 파서 → hwp5txt(pyhwp) → 한컴 COM(Windows + 한컴오피스 + pywin32). 배포용(DRM) HWP 는 한컴 COM 만 가능.
- 다음: attach 가 끝나면 `git pull` → `research` → `dedup` → `attach`(새로 늘어난 공고만 추가 다운로드) → `extract --export` 로 30~50건 시범 → `excel`.

## 0-9. 2026-09-10 attach 완료(사용자 PC, 11:18~14:54 약 3.6시간)
- 공고 4,117건 / 파일 8,377개 / **텍스트 확보 3,531건(86%)**. 형식별: .hwp 4,753/4,758, .hwpx 237/237, .pdf 534/668(실패는 스캔 도면),
  .xlsx 1,673/1,690, .xlsm 79/83, .zip 380/400, .xls 0/513(미지원, 내역서라 불필요), .doc/.xlsb/.pptx/.7z/.egg 소수 미지원.
  → HWP 파서는 실전에서 문제없음. 로그의 HWP 실패는 zip 안의 표지·간지(본문 없음)와 OLE 아닌 파일.
- 추가: `attach` 끝에 텍스트 없는 공고의 원인 분해(첨부 없음/다운로드 실패/미지원 형식만/스캔 PDF)를 출력하고 `data/notes_notext.parquet` 저장,
  `attach --report`(처리 없이 요약만), `extract --limit N`(건축·고액 순 N건 시범).
- 다음(사용자 PC): `git pull` → `research` → `dedup` → `attach`(새 공고만) → `attach --report` 로 586건 원인 확인 →
  `extract`(견적) → `extract --export --limit 40` 으로 Cowork 시범 → `extract --import` → `excel` 로 스키마 확인 → 본 처리 방식 결정(배치 vs Cowork).

## 0-10. 2026-09-10 저녁 — research·dedup·attach 재실행, extract 견적
- `research`: 6,718건(시설 1,836개). 예비 검색으로 시설 54개 회복, 같은 시설ID 검수 행 5쌍 자동 병합. 남은 미검색 3개('(주)강원랜드 유리온실',
  '(재)경기문화재단 수장고', '구 종합운동장공원')는 법인 표기 접두어·한 어절 이름 처리로 수정(다음 실행에서 해결).
- `dedup`: 대표 4,214행(프로젝트 1,978개). **대표 행 4,214개 중 약 350개는 같은 공고가 두 시설에 배정된 것**(복합 발주). → 06_검증로그에
  '복수 시설 배정' 항목으로 기록하고 dedup 출력에 건수 표시. extract 대상은 공고키 기준으로 중복 제거(비용 약 9% 절감).
- `attach`: 고유 공고 약 3,860건 중 텍스트 3,610건. 텍스트 없는 253건 중 183건이 '추출 실패(.hwp)' 로 표시됐으나 .hwp 전체 성공률 99.9% 와 모순 →
  보고 로직이 추출성공 기록을 보지 않던 것. 수정: '추출 성공 기록은 있으나 텍스트 없음(글자수 0 — 표지·이미지뿐 / 재처리 필요)' 로 구분하고 실패 파일 오류 메시지 상위를 출력.
  **사용자가 `attach --report` 를 다시 실행해 원인을 확인해야 함.**
- `extract` 견적: 3,961건(중복 포함) 입력 7,553만 토큰 → 즉시 $198.6(≈27.8만원), 배치 50% ≈ 14만원, 중복 제거 후 약 12.6만원.
  공고당 평균 약 1.9만 토큰 = `llm.max_input_chars 60000` 상한에 걸린 것이 대부분 → 40000 으로 낮추면 약 1/3 절감(시범 후 결정).
- 다음: `git pull` → `dedup`(복수 배정 건수 확인) → `attach --report` → `extract --export --limit 40` → Cowork 로 llm_out 작성 → `extract --import` → `excel`.

## 0-11. 2026-09-10 밤 — 텍스트 없는 224건의 진짜 원인: HWP 서로게이트 문자 → UTF-8 저장 실패
- `attach --report`(수정판): 텍스트 없는 고유 공고 224건 중 **209건이 "추출 성공 기록은 있으나 텍스트 없음(글자수 수천~수십만)"**, 첨부 없음 13, xls 만 1.
- 원인: HWP 문단 텍스트 파서가 16비트 단위를 `chr()` 로 그대로 붙여 UTF-16 서로게이트(이모지·확장 한자 또는 컨트롤 데이터 잔재)가 고아 문자로 남음 →
  `data/text/*.txt` 저장(UTF-8)에서 UnicodeEncodeError → 같은 try 블록이라 추출성공 표시는 남고 텍스트는 버려짐.
- 수정: 서로게이트 쌍은 결합·고아는 제거(`_hwp_para_text`), 모든 파서 출력에 `sanitize_text`, 텍스트 파일 저장 실패는 오류만 기록하고 텍스트 유지,
  zip 임시 파일명 단축(경로 260자 대비), 보고 라벨 통합. 텍스트가 없는 공고는 attach 재실행 때 자동 재처리(파일은 디스크 재사용) → 209건 복구 예상.
- 검색·중복: 고유 공고 3,834건, 복수 시설 배정 347건(06_검증로그).
- 다음(사용자 PC): `git pull` → `attach`(재처리 209건, 수 분) → `attach --report`(텍스트 없음 ≈ 15건이면 정상) → Cowork 시범(이미 내보낸 llm_in 40건 그대로) →
  `extract --import` → `excel`.

## 0-12. 2026-09-10 22:41 — attach 재실행으로 복구 완료, Cowork 시범 진행 중
- `attach` 재실행(수 초): **텍스트 확보 3,820/3,834건(99.6%)**. 남은 14건 = 첨부 없음 13 + xls 만 1 → 복구 대상 아님. HWP 4,829/4,834.
- 사용자가 Cowork 에 프로젝트 폴더를 지정하고 PROMPTS.md 세션 6-대안 지시문으로 `data/llm_in` 40건(건축·고액 순) 시범 추출 중.
- 다음: Cowork 완료 → `extract --import`(문제 파일 목록·미추출 건수 확인) → `excel` → 04/05/06 시트에서 연면적·총공사비·검증 판정을 보고
  본 처리 방식 결정(배치 API 약 12~13만원 vs Cowork 계속). 복수 시설 배정 347건은 Excel 확인 후 검수 파일에서 정리.

## 0-13. 2026-09-11 — Cowork 시범 40건 결과(공사비DB.xlsx) 검토 → 연면적 빈 원인과 수정
- 시범: 40행(고유 38공고) 추출 반영. 01 시트 연면적 27/40 프로젝트(68%). 나머지 1,938개 프로젝트는 아직 미추출이라 비어 있는 것이 정상.
- 시범 40건 중 연면적이 빈 13건의 원인:
  | 원인 | 건수 | 수정 |
  |---|---|---|
  | 내보내기 시점이 HWP 서로게이트 수정 전 → SH 입찰공고문 본문이 빠지고 입찰서류.zip(서약서)만 Cowork 에 전달(관악문화플라자·마곡16단지·고덕강일·연희연남·염주골·울산시립미술관·The-K제주호텔) | 7 | `attach --retry-failed` 가 오류 기록이 있는 파일도 재처리 |
  | 대표 공고가 정정·변경 차수라 첨부가 600~1,400자짜리 정정공고서뿐(안심뉴타운·일광지구·철원갈말2·킨텍스 앵커호텔·율동지구·인천석남) | 6 | 같은 공고번호의 다른 차수 첨부를 합쳐 처리(`_merge_attachment_slots`), `--retry-failed` 가 미반영 차수 첨부가 있는 공고도 재처리 |
  | 공고문(입찰설명서)에 공사개요가 없음(화성 유스호스텔·화곡초) | 2 | 원천 한계 — 현장설명서가 없으면 공란 |
- `extract --redo-low`: 신뢰도 low 또는 건축인데 연면적 없는 기존 결과를 다시 대상에 포함(export·즉시·배치 공통). `--import` 는 반영한 txt·json 을 `data/llm_done/` 으로 이동.
- 다음(사용자 PC): `git pull` → `attach --retry-failed`(오류 7건 + 차수 병합 대상, 수십 분) → `extract --export --redo-low --limit 40` → Cowork 로 llm_out 작성 → `extract --import` → `excel`
  → 연면적 채움률 확인 후 본 처리(배치) 결정. 06 검증로그는 시범 38건 기준 추정가격 정상 23/오류 3, 기초금액≈추정×1.1 경고 29(부가세 면제 공공주택은 1.1 배가 아님 → 규칙 보완 검토).

## 0-14. 2026-09-11 아침 — git pull 충돌(Cowork 가 HANDOFF.md 수정) → 옛 코드로 --retry-failed 실행됨
- Cowork 세션이 지시문의 "HANDOFF.md 갱신"대로 파일을 고쳐 `git pull` 이 거부됨 → 사용자는 7f5776e 이전 코드로 `attach --retry-failed`(682건, 1h46m) 실행.
  텍스트 확보 3,820/3,834 그대로(서로게이트 7건·차수 병합은 미반영). `--redo-low` 는 인식 안 됨.
- 조치: PC 세션은 `HANDOFF_local.md`(gitignore)에만 기록(PROMPTS·CLAUDE.md 갱신). 차수 병합 재처리 대상을 "대표 텍스트 5,000자 미만이거나
  다른 차수에 공고문·현장설명서류가 있는 공고"로 좁힘.
- 사용자 안내: `copy HANDOFF.md HANDOFF_cowork_backup.md` → `git checkout -- HANDOFF.md` → `git pull` → `attach --retry-failed` → `extract --export --redo-low --limit 40`.

## 0-15. 2026-09-11 낮 — attach --retry-failed(새 코드) 완료, API 즉시 호출로 재추출 검증 단계
- `attach --retry-failed`: 재처리 1,202건(차수 병합 실제 234건, 약 1시간 50분). 파일 9,239개, 텍스트 3,821/3,834(첨부 없음 12·xls 1).
  HWP 5,232/5,239. 정정공고 차수가 대표였던 공고들도 원 공고문·현장설명서 텍스트를 갖게 됨.
- `extract --export --redo-low --limit 40`: 다시 할 15건 + 고액 건축 → llm_in 40건 내보냄(Cowork 용이나, 사용자 결정으로 Cowork 대신 API 경로로 진행).
- 다음(사용자 PC): `setx ANTHROPIC_API_KEY` → 새 창 → `git pull`(aecba8c: --redo-low 를 --limit 앞에, 1d7bd16: --import 가 API 결과를 덮어쓰지 않음) →
  `doctor` → `extract --yes --redo-low --limit 15`(즉시, 약 $1) → `excel` → 01 시트 연면적 확인 → `extract --batch --yes`(나머지 약 3,780건, 약 12~13만원).

## 0-16. 2026-09-11 13:30 — API 즉시 호출로 15건 재추출 성공, 구조화 출력 스키마 400 수정
- `extract --yes --redo-low --limit 15`: 15/15 성공(입력 29만 토큰, 약 $0.8). 01 시트 연면적 **32/40**(이전 27). 안심뉴타운 37,208·일광지구 76,632·
  철원갈말2 9,990·킨텍스 앵커호텔 30,500·율동지구 36,699·울산시립미술관 12,771 채워짐(차수 병합·서로게이트 수정 효과 확인).
  남은 8건: SH 입찰공고문에 연면적이 없는 공고 5(관악문화플라자·마곡16단지·고덕강일·연남동·염주골), 입찰설명서만 있는 2(화성 유스호스텔·화곡초), 리모델링 1(The-K제주호텔) → 원천 한계.
- 매 호출이 `400 Schemas contains too many parameters with union types` 로 구조화 출력이 거부되고 일반 파싱으로 폴백했음(요청 2배).
  원인: API 한도 "union 타입(["number","null"]) 파라미터 16개/요청" vs 스키마 19개. 수정: 모든 필드를 string(빈 값 "")으로 받고 normalize_doc 이
  숫자로 변환(union 0개). 거부 시 프로세스 단위로 기억해 반복 400 방지. **배치는 요청이 통째로 실패하므로 이 수정 후에 제출해야 함.**
- 다음(사용자 PC): `git pull` → `extract --batch --yes`(약 3,780건, 약 12~13만원) → 1시간~24시간 뒤 `extract --batch`(수거) → `excel`.

## 0-17. 2026-09-12 02:17 — 배치 수거 완료(본 추출)
- 배치 8개(3,783건, 실제 입력 8,962만 토큰·출력 800만 토큰) → **성공 3,573 / 실패 210**(API 오류 22 + 응답 해석 실패 188 추정). 대표 공고 4,214행 중 추출 완료 3,975.
  검증로그 21,690건: 정상 16,375 / 참고 2,844 / 경고 2,387 / 오류 55 / 미확인 29. `excel` 생성 완료.
- 추가: 실패 원인 요약(`extract`·`extract --batch` 출력에 `_error` 상위), 배치 해석 실패 메시지에 stop_reason·응답 앞부분 포함,
  `extract --batch --refetch`(끝난 배치 결과를 재과금 없이 다시 내려받아 실패 건만 다시 해석 — 파서 수정 뒤 복구용).
- 다음: 사용자 PC `git pull` → `extract --batch --refetch` 로 실패 원인 확인 → 원인별 조치(잘림이면 max_tokens 상향 후 실패 건만 재제출
  `extract --batch --yes`(실패 건은 자동 재대상), 거부면 export 로 사람 처리) → 전체 Excel 분석(연면적 채움률·06 판정·부가세 면제 규칙 보완).

## 0-18. 2026-09-12 03:00 — 실패 210건 원인 확정(max_tokens 잘림) + 전체 Excel 분석
- `extract --batch --refetch` 결과: **155건 `stop=max_tokens` + JSON 미검출, 6건 `stop=max_tokens` + JSON 중간 잘림, 22건 API 오류**.
  원인: Sonnet 5 는 기본으로 사고(thinking)하며 사고 토큰이 `max_tokens`(8,000)에 포함된다 → 긴 공고에서 사고가 한도를 다 써 JSON 이 나오기 전에 끊김.
- 조치: `config.yaml llm.max_tokens 16000`(기본값 `extract_llm.DEFAULT_MAX_TOKENS`), 이전에 잘린 건은 다음 실행 때 자동으로 2배(32,000)로 재요청
  (`pipeline._truncated_retry_tokens` → `submit_batches(max_tokens_by_key)` / `extract_with_claude(max_tokens)`), 배치 API 오류는 안쪽 원인(type·message)을 기록,
  비용 견적의 출력 토큰을 건당 2,000 으로(실측 평균 2,012).
- Excel(`output/공사비DB.xlsx`, 프로젝트 1,978) 분석: 연면적 756/1,978(38%). 건축공종 보유 1,286 중 연면적 649 / 없음 637(신축 394·리모델링 166·증축 69·증축리모델링 8).
  연면적 없는 건축 대표공고 596건의 첨부는 중앙값 2개(공고문+내역서)이고 **도면·설계도서 첨부는 17건뿐** → 도면 PDF 처리로는 회수 불가(사용자 질문에 답변).
  06 판정: 정상 16,375 / 경고 3,437 / 참고 2,844 / 오류 55(기초금액 41·추정가격 14) / 미확인 29. 경고 상위: 총공사금액=기초금액+관급 1,049, 소액 공고 분리 587, 사업유형 일치 513, 재발주 금액변동 442, 기초금액≈추정가격×1.1 393.
  05 시설합산의 연면적은 01 을 INDEX/MATCH 로 참조하는 수식(pandas 로 읽으면 빈 값으로 보이나 정상).
- 다음: 사용자 PC `git pull` → `extract --batch --yes`(실패 210건만 자동 재제출, 잘린 161건은 32,000 토큰) → 끝나면 `extract --batch` → `excel`.
  연면적 보강은 도면이 아니라 다른 출처(설계용역·설계공모 공고의 과업개요, 사전규격 첨부)를 검토해야 한다(6절).

## 1. 현재 단계
- **세션 0(코드 정비) 완료 / 세션 1(환경 점검 + probe)의 사용자 PC 실행 대기**
- 이번 작업은 네트워크가 막힌 샌드박스(Claude Code 웹)에서 진행되어 **나라장터 API 호출(probe/collect)은 실행하지 못했다.**
  API 키(`G2B_SERVICE_KEY`)도 설정되어 있지 않았다. 따라서 `probe` 이후 단계는 모두 사용자 PC(Windows)에서 진행해야 한다.

## 2. 이번 세션에서 한 일
- 파이프라인을 전용 저장소 **github.com/seio0701/g2b_costdb** 로 분리(research-eval-web 과 무관한 별도 프로젝트). 설계 문서는 `docs/나라장터_공사비DB_자동화_울트라플랜.md`, PC 설치 절차는 `docs/PC_설치_및_첫실행_가이드.md`.
- 합성 데이터 테스트(`python -m tests.test_dedup_and_excel`) 통과 확인 → `sample_output/공사비DB_샘플.xlsx` 생성(수식 오류 0건, `formulas` 패키지로 평가).
- 실데이터 첫 실행에서 터질 결함을 모듈별 적대적 검토(7개 관점 × 재현 검증)로 찾아 수정. 주요 항목:
  | 영역 | 수정 |
  |---|---|
  | API | 포털 오류 XML(키 미등록 30, 트래픽 초과 22 등)을 해석해 재시도 없이 한국어 안내로 중단, 22는 "내일 재개"로 처리, NODATA(03)는 빈 결과. Encoding 키(`%2B…`) 자동 변환. totalCount 누락 시에도 끝까지 페이지 조회 |
  | 수집 | 오늘이 속한 달은 완료 표시 없이 매번 재수집(캐시 우회), 미래 달 건너뜀, done.txt 에서 지운 달은 새로 받음, 월별 실패는 건너뛰고 마지막에 `[미완료]` 보고. 월별 타입이 달라도 parquet 저장 실패 없음 |
  | 탐색·검수 | 시설 후보 키에 수요기관 포함(동명 시설 분리), `discover` 재실행 시 검수 내용·시설ID 유지, `검수_수요기관` 열 추가(재검색 범위 제한), 빈 검수_시설명 처리, 한 공고가 두 분류에 걸리면 구체적 검색어 1건만 채택, '제주시' 접두어 보존, 띄어쓴 검색어('온실 건립') 처리 |
  | 분류 | '토목건축공사업'은 건축, '소방서' 건물은 소방 아님, '(취소 후 재공고)'를 취소로 오인하지 않음, 매입임대·복합화 규칙 정리 |
  | 중복정리 | 같은 공고가 두 시설에 걸려도 탈락 없음, 괄호 안 (1단계)/(2공구) 분할발주 토큰 인식, 'N차 재공고'는 회차로 처리, 날짜 없는 공고를 최신으로 오인하지 않음, **추정가격이 최대치의 30% 미만인 공고(무대기계·승강기 설치 등)는 본공사 대표를 대체하지 못함**(경고 기록) |
  | 첨부 | 다운로드 중단 파일 캐시 방지(.part), HTML(로그인) 응답 감지, 기존 파일은 요청 없이 재사용, 확장자 없으면 매직바이트로 보완, HWPX 탭 뒤 텍스트 보존, cp949 파일명·텍스트, 공고 단위 오류 격리와 25건마다 저장(중단 후 이어서) |
  | LLM 추출 | JSON 스키마 강제(구조화 출력, 미지원 시 폴백), 금액 문자열·NaN 안전 처리, 실패 건은 다음 실행에서 재시도, 인증 실패 즉시 중단, **`extract` 는 비용 견적만 출력하고 `extract --yes` 로만 실제 호출**, API 금액을 힌트로 주지 않아 교차검증 독립성 확보 |
  | Excel | API 금액이 없을 때 문서 추출 금액으로 총공사비 산출, 공종누락경고를 '누락'(공고 없음)/'금액없음'으로 구분, 05 연면적은 01 시트(노란 셀) 참조, 검증로그 숫자 컬럼 통일 |
  | CLI | `doctor` 단계(파이썬·패키지·키 존재·네트워크 점검), `config.yaml paths` 를 프로젝트 폴더 기준으로 해석(어느 폴더에서 실행해도 동일), Excel 파일이 열려 있을 때 안내, cp949 콘솔 보호 |
- 회귀 테스트 `tests/test_regressions.py` 추가(위 수정사항 대부분을 검증), `tests/check_excel.py`(수식 오류 점검) 추가.
- **가짜 나라장터 서버(`tests/mock_g2b.py`) + 통합 테스트(`tests/test_e2e_mock.py`)** 추가: probe(잘못된 키 → 30 오류 안내, 정상 키 → 매핑 OK·면허제한 필드 후보 안내) → collect(데이터 없는 달 03, 재실행 시 호출 0) → discover(시설 3개, 재실행 시 ID 유지) → research(lcnsLmtNm 자동 탐색, 유지보수 제외, 수요기관 범위) → dedup(변경차수·재공고·소액 분리) → attach(HWPX/DOCX/cp949 TXT, HTML 실패 기록) → extract(견적 → 모의 --yes) → excel(수식 오류 0) 전 단계 통과.
- 문서 갱신: README.md(실행 순서·doctor·extract --yes), CLAUDE.md(단계·성공기준), PROMPTS.md(세션 1·6 지시문), config.yaml(새 설정 `api.license_query`, `dedup.minor_notice_ratio`, `llm.structured_output`, `llm.usd_krw`, `max_tokens 8000`).

## 3. 생성·변경된 파일
- 코드: `g2b_costdb/api_client.py`, `collect.py`, `classify.py`, `discover.py`, `dedup.py`, `attachments.py`, `extract_llm.py`, `build_excel.py`, `pipeline.py`
- 설정: `config.yaml`, `keywords.yaml` · 테스트: `tests/test_regressions.py`, `tests/check_excel.py` · 문서: `README.md`, `CLAUDE.md`, `PROMPTS.md`, `docs/…울트라플랜.md`
- 데이터 산출물(`data/`, `output/`)은 아직 없음(API 미실행). `sample_output/공사비DB_샘플.xlsx` 는 가상 데이터 예시.

## 4. 다음 할 일 (사용자 PC, PROMPTS.md 세션 1)
1. PowerShell 에서 `git clone https://github.com/seio0701/g2b_costdb.git C:\DB_WORK\g2b_costdb` → `cd C:\DB_WORK\g2b_costdb` → `pip install -r requirements.txt` → `pip install pyhwp`
2. 공공데이터포털에서 「조달청_나라장터 입찰공고정보서비스」 활용신청 후 **일반 인증키(Decoding)** 를 `setx G2B_SERVICE_KEY "키"` 로 설정(새 터미널 열기)
3. `python -m g2b_costdb.pipeline doctor` → 모두 OK 인지 확인
4. `python -m tests.test_dedup_and_excel` → `OK` 두 줄 확인, `python -m tests.test_e2e_mock` → `OK: 통합 실행 통과` 확인
5. `python -m g2b_costdb.pipeline probe --ym 2026-08` → `[매핑 확인 필요]` 항목이 있으면 `config.yaml fields` 수정. 면허제한 표본에서 업종명 필드 안내(`lcnsLmtNm` 등)가 나오면 `fields.license_name` 수정, 오류 10/11 이면 `api.license_query.inqryDiv` 조정
6. 이후 PROMPTS.md 세션 2(3개월 시험 수집: `period` 를 2026-06~2026-08 로) → 세션 3(본 수집) 순서

## 5. 사용자 결정 필요 사항
- LLM 추출 모델: 기본 `claude-sonnet-5`(저비용). 정확도를 우선하면 `config.yaml llm.model` 을 `claude-opus-5` 로(비용 약 2.5배). `extract`(플래그 없음)가 견적을 보여준다.
- `dedup.minor_notice_ratio`(기본 0.30): 같은 프로젝트·공종에서 추정가격이 최대치의 30% 미만인 공고를 부대공사로 보고 대표에서 제외한다. 소규모 시설(온실·재난물자창고)에서 본공사가 잘못 제외되면 06_검증로그의 '소액 공고 분리' 항목을 보고 값을 낮춘다.

## 6. 미해결·주의
- 실제 API 응답 필드명·면허제한 조회 파라미터는 `probe` 로 확정 전까지 추정값이다(코드는 미존재 필드를 공란 처리하고 업종명 필드 후보를 자동 탐색).
- HWP 파서는 규격 기반 합성 데이터로만 검증했다. 첫 `attach` 실행 후 `07_추출노트`의 HWP 실패율을 확인할 것(20% 초과 시 pyhwp/한컴 COM 폴백 점검).
- 나라장터 첨부 URL이 로그인/세션을 요구하면 다운로드가 'HTML 응답' 실패로 기록된다. 이 경우 수동 다운로드 후 `data/files/<공고번호>/` 에 넣고 `attach` 재실행.
- 연면적 공백(건축 637 프로젝트)은 공고 첨부 자체에 정보가 없는 경우가 대부분(공고문+내역서만 첨부). 도면 처리로는 17건 이하만 회수 가능하므로 보강 출처 후보:
  ① 같은 시설의 건축설계 용역·설계공모 공고(입찰공고정보서비스 용역 오퍼레이션, 과업개요에 연면적 명시가 흔함) ② 사전규격 첨부(사전규격번호 보유) ③ 건축물대장 API(지번 필요, 난이도 높음).
  어느 것을 할지는 사용자 결정 사항(추가 API 호출·비용 수반).
