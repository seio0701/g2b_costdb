...
7. (선택) 낙찰금액·낙찰률 참고 컬럼도 원하면 **`조달청_나라장터 낙찰정보서비스`** 를 검색해 같은 방법으로 활용신청한다.
   인증키는 같은 것을 쓴다(따로 설정할 것 없음). 승인 뒤 `config.yaml` 의 `use_awards: false` 를 `true` 로 바꾸면 probe·collect 가 낙찰 목록도 다룬다.
   서비스 페이지의 **End Point** 주소가 `config.yaml api.award_base_url` 과 다르면 그 주소로 바꾼다. 나중에 해도 된다(이미 받은 공고 데이터는 그대로 두고 `collect` 만 다시 실행).
...
### 7-5. 낙찰정보 덩어리 (`use_awards: true` 로 켰을 때만 나옴)
```
=== getScsbidListSttusCnstwk params={'inqryDiv': '1'} (2026-08) totalCount=... 표본 5건 ===
필드: [...]
  [award_amt] OK                      ← 또는 "후보 scsbidAmt 발견 → config.yaml fields.award_amt 를 이 값으로"
  [award_rate] OK
  ...
[낙찰정보] 기간 조회 파라미터 {'inqryDiv': '1'} 로 결과 확인 → ...
```
- `실패: API 오류 20` 또는 `30` 이 낙찰 덩어리에서만 나오면: 낙찰정보서비스 활용신청이 아직 승인되지 않았거나 `api.award_base_url` 이 서비스 페이지의 End Point 와 다른 경우다. 3단계 7번을 확인한다. 확인 전까지는 `use_awards: false` 로 두면 된다(공고·기초금액 수집에는 영향 없음).
- `후보 ... 발견` 이 나오면 7-1 과 같은 방법으로 `fields:` 의 `award_...` 줄을 그 이름으로 바꾼다(안 바꿔도 후보를 자동으로 쓴다).
- `어떤 조회구분으로도 항목이 없음` 이면 probe 출력을 통째로 Claude Code 에 붙여 넣고 "낙찰정보 조회 파라미터를 맞춰 달라"고 한다.
