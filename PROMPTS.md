...
> PC 에서 실행하는 Claude Code·Cowork 세션은 진행 기록을 **HANDOFF_local.md** 에 쓴다. HANDOFF.md 는 원격(웹) 세션이 git 으로 갱신하므로
> PC 에서 고치면 `git pull` 이 "local changes would be overwritten" 으로 막힌다. 그때는 `git checkout -- HANDOFF.md` 로 되돌린 뒤 다시 pull.
...
5) 끝나면 HANDOFF_local.md 에 진행상황을 정리해줘(HANDOFF.md 는 고치지 말 것).
```
...
표2 검색어(keywords.yaml)로 잡힌 결과가 상식적으로 맞는지 네 의견도 짧게 덧붙여줘. HANDOFF_local.md 에 진행 상황을 덧붙여줘(HANDOFF.md 는 고치지 말 것 — git pull 충돌 방지).
```
...
낙찰금액·낙찰률은 참고 컬럼이고 총공사비 기준은 그대로 추정가격·기초금액이야. HANDOFF_local.md 에 진행 상황을 덧붙여줘(HANDOFF.md 는 고치지 말 것 — git pull 충돌 방지).
```
...
"예산 도달"로 멈추면 우회하지 말고, 오늘까지 완료된 월과 남은 월 수를 알려주고 끝내줘. HANDOFF_local.md 에 진행 상황을 덧붙여줘(HANDOFF.md 는 고치지 말 것 — git pull 충돌 방지).
```
...
내가 노란 셀(검수_포함여부/검수_시설명/검수_별칭)을 직접 채울 거니까 그 파일은 수정하지 마. HANDOFF_local.md 에 진행 상황을 덧붙여줘(HANDOFF.md 는 고치지 말 것 — git pull 충돌 방지).
```
...
data/text 폴더의 txt 3개를 열어 공사개요·추정가격 문구가 보이는지 확인해서 알려줘. HANDOFF_local.md 에 진행 상황을 덧붙여줘(HANDOFF.md 는 고치지 말 것 — git pull 충돌 방지).
```
...
3) 내가 "배치 진행"이라고 하면 extract --batch --yes 로 제출하고, 1시간 뒤(또는 다음 세션에서) extract --batch 로 결과를 수거해줘.
   "바로 진행"이라고 하면 extract --yes 로 즉시 처리해줘. 끝나면 추출 성공 건수와 06_검증로그 기준 정상/경고/오류 건수, 경고 상위 10건을 보여줘. HANDOFF_local.md 에 진행 상황을 덧붙여줘(HANDOFF.md 는 고치지 말 것 — git pull 충돌 방지).
```
...
전부 끝나면 python -m g2b_costdb.pipeline extract --import 를 실행해 반영하고, 문제 파일 목록과 미추출 건수를 알려줘. HANDOFF_local.md 에 진행 상황을 덧붙여줘(HANDOFF.md 는 고치지 말 것 — git pull 충돌 방지).
...
이상치(㎡당 공사비가 다른 시설의 3배 이상이거나 1/3 이하)는 원인 후보(연면적 오독, 공종 누락, 관급 누락)와 함께 정리해줘. HANDOFF_local.md 에 진행 상황을 덧붙여줘(HANDOFF.md 는 고치지 말 것 — git pull 충돌 방지).
```
